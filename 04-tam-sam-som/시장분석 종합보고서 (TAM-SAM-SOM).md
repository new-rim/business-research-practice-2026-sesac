...
> 근거 문서: `보상형 그룹별 일상 숏폼 공유 플랫폼(아이디어).md` · `SAM 시장 세분화 맵 v2 (아이디어 정합).md`
...
> **구조적 특징** — 보상 재원에 **외부 광고주가 없다.** 사용자가 낸 돈이 사용자에게 돌아가고 플랫폼은 그 흐름에서 수수료를 뗀다. Zenly형 "수익모델 부재로 종료" 리스크가 구조적으로 제거되는 대신, **제로섬 구조**라 신규 사용자에게 "돈을 잃을 수 있다"는 진입 장벽이 생긴다.
>
> **완화 프레이밍** — 참가비는 *"거는 돈"*이 아니라 *"나에게 맡기는 돈"*이다. 100% 달성 시 전액 환급이므로 성실한 사용자에게 손실은 0이다.

### 1.5 최대 리스크

| 우선순위 | 리스크 | 대응 |
|---|---|---|
| 🔴 진입 전 필수 | **사행성 규제** — 참가비를 걸고 결과에 따라 재분배 | **우연성 배제 논리**: 결과가 전적으로 본인 행위로 통제되고 100% 달성 시 전액 환급이면 사행이 아니라는 구성. 챌린저스 선례 확인 필요 → **법률 검토 선행** |
| 🔴 진입 전 필수 | **전자금융거래법** — 예치금 보관 + 포인트 현금 전환이 선불전자지급수단 해당 가능 | 등록 요건·예치금 분리 보관 확인. PG·에스크로 위탁 검토 |
| 🟡 MVP 중 | **어뷰징** — 현금이 걸리면 반드시 발생 | 실시간 촬영만 허용 · 영상 해시 중복 검사 · 본인 인증 · 랜덤 샘플 심사 |
| 🟡 MVP 중 | 세무 — 상금 지급 시 기타소득 원천징수 | 지급액 임계 설계 + 자동 원천징수 |
