...
> | 항목 | 값 |
> |---|---|
> | **문서 ID** | `value-proposition-0814` — **PRD의 입력 기준본** (08단계 자립 확정본 `VPS-v2.0-rooted`와 동일 내용, 2026-08-14 시점 스냅샷) |
...
> | **흡수한 선행 리서치** | 04 시장분석·아이디어 정의서 / 05 페르소나·스펙트럼·여정지도 / 06 Pain 인벤토리·AOS·DOS·사분면 / 07 JTBD 인터뷰 설계·모의 9건 / 08 경쟁사 9종·가치목표 3안 / **법률검토** Phase 0 리스크 브리프 |
...
> | 분량 | 약 1,840행 — **자립성의 대가로 길어졌다.** 목차와 절별 요약 박스로 진입점을 만들었다 |
...
>
> **경쟁사 분석(§4)만은 성격이 다르다** — 공개 재무·통계에 기반하며 출처 링크 52건이 HTTP 검증을 통과했다(§4.4에 전량 수록).
>
...
| **1** | [페르소나·CJM 기반 고객별 핵심 문제 (Pain, Needs)](#1-페르소나cjm-기반-고객별-핵심-문제-pain-needs) | 페르소나 14인 카드 · 여정 7단계 · **Pain 42건 전량** |
| **2** | [JTBD 선언 — 상황에 따른 목표 (Goal, Job)](#2-jtbd-선언--상황에-따른-목표-goal-job) | 인터뷰 설계·금지 규정 · 9인 선정 근거 · 데모 6화면 체류 원표 |
| **3** | [고객이 원하는 Outcome (측정 가능 형태)](#3-고객이-원하는-outcome-측정-가능-형태) | AOS·DOS **계산식과 42건 전량 점수** · 사분면 |
| **4** | [기존 대안 (Competitor / Substitute)](#4-기존-대안-competitor--substitute) | 경쟁 9종 상세 · 무료 대체재 · **출처 52건** |
...
### 1.0 페르소나 스펙트럼 14인 — 누가 이 문제를 겪는가
...
>
> **함께 검증할 파일럿 항목** — V1 *"달성에 붙이면 관계는 안전하다"*(원안 vs 확정안 A/B 셀, 확정안의 W4 리텐션이 유의하게 높을 것) · V2 *"친구끼리 돈이 오가도 관계가 불편해지지 않는다"*(참가 전후 관계 만족도 설문, 유의한 하락 없을 것) · V3 계약형 알림의 알림 OFF 비율 · V4 보상 수용률 55%(옵트인율 **50% 이상**) · **F1의 "친구 동반 실천 40%"**(문체부 국민생활체육조사 원자료로 실측 대체 — **가장 약한 고리**).
...
10. **시장 모수의 가장 약한 고리는 F1 108만의 "친구 동반 실천 40%" `[러프]`다.** 국민생활체육조사가 참여 동반자 세부 비율을 비공개해 추정에 의존했다.
