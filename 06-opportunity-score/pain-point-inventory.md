# 06-1. 대표 Pain Point 인벤토리 — AOS 평가 대상 42건

> 작성일: 2026-08-11
> **문서 위치:** `05-personas/customer-journey-map.md`(14명 × 7단계)에서 **인물별 대표 Pain Point 3건씩, 총 42건**을 추출하고 **각각의 Goal을 함께 도출**해 정렬한 문서입니다. 다음 단계인 **AOS 채점의 입력 자료**가 됩니다.

| 선행 문서 | 여기서 가져오는 것 |
|---|---|
| `05-personas/customer-journey-map.md` | 14명 × 7단계 Pain Point 원본 |
| `05-personas/persona-02-spectrum.md` | 각 인물의 상위 Goal |
| `06-opportunity-score/methodology.md` | AOS 정의 · 사분면 해석 · 평가 대상 정의 |

---

## 0. 어떻게 골랐나

여정지도에는 **14명 × 최대 7단계 ≈ 90건 이상**의 Pain Point가 있습니다. 전부 채점하면 우선순위가 흐려지므로 **인물마다 3건씩만** 남겼습니다.

### 선별 기준 — 아래 순서로 적용

| 우선 | 기준 | 왜 |
|:-:|---|---|
| 1 | **여정을 끊거나 배제하는 것** (⛔) | 이탈·배제는 매출에 직결됩니다 |
| 2 | **그 사람의 다른 통증의 원인이 되는 것** | 원인을 고치면 결과가 함께 풀립니다 |
| 3 | **여러 사람이 공유하는 것** | 하나를 고치면 여러 명이 복구됩니다 |
| 4 | 통증 강도가 가장 센 것 | 위 셋으로 정해지지 않을 때만 |

### Goal은 어떻게 도출했나

방법론의 AOS 정의는 *"고객에게 **Pain/Goal**이 얼마나 중요한가"*를 Importance로 봅니다. 즉 **Pain만으로는 중요도를 매길 수 없습니다** — 그 Pain이 **무엇을 막고 있는지**를 알아야 합니다.

그래서 두 층위의 Goal을 함께 적었습니다.

| 층위 | 무엇인가 | 어디서 왔나 |
|---|---|---|
| **인물 Goal** | 그 사람이 이 서비스에서 궁극적으로 이루려는 것 | 페르소나 문서의 `목표` 항목 |
| **Pain별 Goal** | **그 순간, 그 화면에서** 이루려던 것 | 여정지도의 행동·생각·감정에서 역산 |

> **Pain은 "무엇이 막혔나", Goal은 "무엇을 하려다 막혔나"입니다.** 둘을 나란히 놓아야 *"이 사람에게 얼마나 중요한가"*를 판단할 수 있습니다.
...
> ⚠️ **아직 점수는 없습니다.** 이 문서는 채점 **대상 목록**입니다. Importance·Satisfaction 값을 매겨 AOS로 내림차순 정렬하는 것은 다음 단계(§6)입니다.
...
**③ Goal이 제품과 반대 방향인 사람이 있습니다 — 채점하면 안 됩니다.**

37·38번(서지훈)의 Goal은 **"이 앱을 안 쓰는 것"**입니다. 이 사람에게 이 Pain은 매우 중요하고(Importance 높음) 전혀 해결되지 않았습니다(Satisfaction 낮음). 수식에 그대로 넣으면 **AOS 최상위권**이 나옵니다.

> 하지만 그건 혁신 기회가 아니라 **채점 대상 선정 오류**입니다. AOS는 *"우리가 풀면 고객이 좋아할 문제"*를 찾는 도구인데, 이 Goal은 **우리가 풀수록 고객이 멀어지는** 종류입니다.
>
> **✅ 후속 정정 (`06-2 AOS 채점 결과` §5 해설 ①)** — 실제로 채점해 보니 **37·38번은 AOS 0.0으로 최하위**가 나왔습니다. Goal이 *"안 쓰는 것"*인데 현재 안 쓰고 있으므로 **Satisfaction이 5점**이 되어 수식이 자동으로 0을 만듭니다. **위 경고는 Pain 기준으로 채점할 때만 성립하며, Goal 컬럼을 붙인 것이 문제를 미리 해소했습니다.**

**④ 인물 Goal과 Pain별 Goal이 어긋나는 곳이 선별 오류 후보입니다.**

3건씩 뽑은 게 맞는지는 **그 사람의 상위 Goal을 향하고 있는지**로 검증됩니다.

- **강수빈**은 정확히 맞습니다 — 상위 Goal(*불규칙한 삶에서도 유지 · 실패가 근무표 탓임을 인정받기 · 잃은 만큼 회복*)이 31·32·33번과 **1:1로 대응**합니다.
- **송재현**도 맞습니다 — *기간 없는 꾸준함을 누적*이라는 상위 Goal 아래 25·26·27이 각각 시작·축적·유지를 맡습니다.
- **한지우**는 조금 어긋납니다. 상위 Goal은 *"판정자 자리에서 내려오는 것"*인데, 뽑힌 3건(16 이주·17 관행 충돌·18 재원 상실)은 **전부 도입과 운영의 부작용**입니다. 정작 *"판정에서 해방되는 순간"*은 여정지도의 S4에 있는데 3건에서 탈락했습니다 → **재선별 후보**입니다.
...
### 5-2. 여러 사람이 공유하는 묶음 — 먼저 채점할 대상

> **하나를 고치면 여러 명이 동시에 복구되므로, AOS 채점을 이 묶음부터 시작하는 것이 효율적입니다.**
...
이 42건에 **Importance(1~5)**와 **Satisfaction(1~5)**을 매겨 `AOS = Importance × (1 − Satisfaction/5)`로 계산하고 **내림차순 정렬**합니다.

### 채점표 서식

| # | 인물 | 단계 | Pain Point | Goal | Importance<br/>(1~5) | Satisfaction<br/>(1~5) | 1−Sat/5 | **AOS** | 사분면 |
|:-:|---|:-:|---|---|:-:|:-:|:-:|:-:|:-:|
| 01 | 정하윤 | S3·S4 | 독촉 역할 재발 | 잔소리하지 않고 그룹이 굴러가게 | | | | | |
| … | | | | | | | | | |
...
| # | 결정 사항 | 왜 먼저 정해야 하나 |
|:-:|---|---|
| 1 | **Importance를 무엇 기준으로 매길 것인가** | Pain이 아니라 **Goal의 중요도**로 매겨야 합니다(방법론 정의). 그리고 그 Goal이 **페르소나 본인 기준인지 사업 관점인지**에 따라 순위가 완전히 달라집니다 |
| 2 | **Satisfaction의 기준선은 어디인가** | *"기존 대체 솔루션이 이 Goal을 얼마나 이뤄주는가"*로 잡아야 합니다 — 우리 제품 기준으로 잡으면 아직 제품이 없어 측정 불가입니다 |
| 3 | **비활성 사용자(37~42)를 채점 대상에 넣을 것인가** | 🔴 **37·38번은 Goal 방향이 제품과 반대**라 그대로 채점하면 최상위로 올라옵니다(§3 해설 ③). 제외하거나 별도 트랙으로 다뤄야 합니다 |
| 4 | **사분면 경계값을 어디로 둘 것인가** | 방법론 문서가 참고 링크로만 남겨둔 열린 질문입니다 |
...
2. **한지우는 재선별 후보입니다.** §3 해설 ④에서 본 대로, 뽑힌 3건이 상위 Goal(*판정자 자리에서 내려오기*)을 정면으로 다루지 않습니다.
