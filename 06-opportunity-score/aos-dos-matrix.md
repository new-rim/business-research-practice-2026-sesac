...
> 작성일: 2026-08-11
> **문서 위치:** `06-2 AOS 채점`의 고객 절실도와 `06-3 DOS 채점`의 시장 가중 점수를 **두 축으로 교차 배치**한 매트릭스입니다. DOS 두 버전(SAM·SOM)에 대해 각각 그렸습니다.

| 선행 문서 | 가져온 것 |
|---|---|
| `06-opportunity-score/aos-scoring.md` | AOS 값 (X축) |
| `06-opportunity-score/dos-scoring.md` | DOS 값 SAM·SOM 두 버전 (Y축) |
...
> **경계선 값(중앙값 그 자체)은 Low에 포함**했습니다 — `06-2`와 동일한 규칙입니다.
...
> **모수를 무엇으로 잡든 최우선 4건은 바뀌지 않습니다.** 06-3에서 *"모수 논쟁의 실질은 C2를 시장으로 셀 것인가 하나뿐"*이라고 했는데, **그 논쟁이 최우선 판단에는 영향을 주지 않는다**는 것이 여기서 확인됩니다.
>
...
| # | 항목 | SAM | SOM | 왜 |
|:-:|---|:-:|:-:|---|
| `21` | 윤채원 전환 트리거 | 📈 Q2 | ⚫ Q3 | C2 분자가 428만 → 143만으로 줄어 DOS 0.54 → 0.62이나 중앙값이 더 크게 올라감 |
...
> 그런데 이 셋은 **"아직 안 풀린 문제"가 아니라 "우리 제품이 새로 만드는 문제"**입니다(06-2 §5 해설 ③). 기존 대체 솔루션에서는 잘 충족되고 있어 점수가 낮게 나올 뿐입니다.
>
