...
| 페르소나 | **S0**<br>자각 | **S1**<br>초대·합류 | **S2**<br>계약 | **S3**<br>예치 | **S4**<br>인증 | **S5**<br>정산 | **S6**<br>회고·재계약 | 종착 지점 |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|---|
...
### 2-4. Core 감정 곡선 — 정하윤(캡틴) vs 김도현(러너)

#### 🔵 C-01 **정하윤** (26) · 크루 캡틴 — 그룹을 여는 사람

```mermaid
xychart-beta
    title "정하윤 (크루 캡틴) 감정 곡선 : 1 최저 ~ 5 최고"
    x-axis ["S0 자각", "S1 초대", "S2 계약", "S3 예치", "S4 인증", "S5 정산", "S6 회고"]
    y-axis "감정 수준" 1 --> 5
    line [3, 4, 2, 2, 3, 4, 4]
```

#### 🔵 C-02 **김도현** (24) · 크루 러너 — 초대받는 사람

```mermaid
xychart-beta
    title "김도현 (크루 러너) 감정 곡선 : 1 최저 ~ 5 최고"
    x-axis ["S0 자각", "S1 초대", "S2 계약", "S3 예치", "S4 인증", "S5 정산", "S6 회고"]
    y-axis "감정 수준" 1 --> 5
    line [2, 4, 3, 1, 1, 2, 2]
```
...
| 단계 | **정하윤** · 크루 캡틴 (그룹을 여는 사람) | **김도현** · 크루 러너 (초대받는 사람) | 격차 |
|---|---|---|:-:|
| **S0 자각** | 3 — 결심 + 자기혐오 | 2 — 체념 | 1 |
| **S1 초대·합류** | 4 — 기대 + 눈치 | 4 — 반가움 | 0 |
| **S2 계약** | 2 — 조율 피로, 시작 전 소진 | 3 — 눈치 (반대하면 분위기가 깨짐) | 1 |
| **S3 예치** | 2 — 부담 + 독촉자 역할 기시감 | **1 — 경계 · 손실 공포** | **1** |
| **S4 인증** | 3 — 안도 ↔ 감시자 감각 재발 | **1 — 죄책감 → 회피** | **2** |
| **S5 정산** | 4 — 성취 + 죄책감 | 2 — 억울함 + 자책 | **2** |
| **S6 회고·재계약** | 4 — 뿌듯 + 다음 시즌 부담 | 2 — 위축 | **2** |

> **두 곡선이 S4~S6 구간에서 최대로 벌어진다.**
>
> **정하윤(캡틴)**은 S2 계약 조율에서 한 번 꺾였다가 완주하며 회복한다. 반면 **김도현(러너)**은 **S3 예치에서 바닥을 치고 끝까지 회복하지 못한다.**
>
> 같은 제품, 같은 그룹, 같은 화면인데 정반대의 곡선이 나온다 — 이것이 **Core를 하나의 여정으로 그리면 안 되는 이유**다.
...
| 단계 | 걸리는 사람 | **공통 Pain Point (단계의 본질적 통증)** | 성격 |
|---|---|---|---|
| **S0 자각** | **서지훈**(알림 이탈자) 이탈 | **카테고리 낙인** — 설명 이전에 분류가 끝난다 | 인식 |
| **S1 초대·합류** | **이준호**(루틴 커미터) · **홍다인**(앱테크 루틴러) · **한지우**(벌금 총무) 마찰 | **그룹 제품인데 그룹 없이 도착하거나(이준호·홍다인), 그룹째 옮겨야 한다(한지우)** | 획득 |
| **S2 계약** | **강수빈**(교대근무 실패자) 배제<br/>**정하윤**(크루 캡틴) · **김도현**(크루 러너) 마찰 | **'변경 불가'가 자기구속 장치이자 배제 장치다** — 합의 비용은 시작 전에 몰리고, 불규칙 생활자는 계약 자체를 쓸 수 없다 | 🔧 설계 |
| **S3 예치** | **김도현**(크루 러너) · **윤채원**(데일리 크루) · **홍다인**(앱테크 루틴러) · **조은비**(현금 구조 거부자) — **4명 이탈** | **'받는 돈 → 거는 돈' 전환 문턱** — 손실 공포(김도현)·관계 화폐화(윤채원)·경험 부재(홍다인)·도덕 판단(조은비)이 **서로 다른 이유로 같은 화면에서 멈춘다** | 💰 **최대 이탈 관문** |
| **S4 인증** | **김도현**(크루 러너) · **박서연**(스터디 페이서) · **강수빈**(교대근무 실패자) 마찰 | **밀린 순간 복귀 비용이 급등하고, 실패 사유가 구분되지 않는다** — 이탈이 무응답 형태라 개입 타이밍이 없다 | 리텐션 |
| **S5 정산** | **임태경**(상습 미달성자) 순환<br/>**정하윤** · **김도현** 관계 손상 | **제로섬이 관계·자기인식 비용으로 청구된다** — 상금의 출처가 아는 사람의 손실이라는 사실이 드러난다 | ⚖️ 윤리·관계 |
| **S6 회고·재계약** | **정하윤**(크루 캡틴) · **김도현**(크루 러너) 약화 | **회고본이 완주자만의 기록이고, 캡틴 역할이 고정된다** — 재계약 동력이 특정 인물과 특정 결과에만 붙는다 | 확산 |
...
| 순위 | 개선 과제 | 고치는 단계 | **누구의 무엇을 푸는가** | 근거 |
|:-:|---|:-:|---|---|
| **1** | **예치 사다리** — 무예치 체험 → 소액 예치 → 정상 예치 | S3 예치 | **김도현**(크루 러너)의 *손실 공포*<br>**홍다인**(앱테크 루틴러)의 *예치 경험 부재* | 최대 이탈 관문이고, S3에서 멈추는 4명 중 2명이 이 처방으로 통과 가능 |
| **2** | **비제로섬 트랙 분리** — 상금 재원을 몰수분이 아니라 구독료에서 조달 | S3 예치<br>S5 정산 | **조은비**(현금 구조 거부자)의 *도덕 판단*<br>**정하윤**(크루 캡틴)·**김도현**(크루 러너)의 *관계 비용* | S3와 S5의 통증을 동시에 제거. 단 수익 구조 재설계가 선행돼야 함 |
| **3** | **주간 총량 계약 + 유동 윈도우** — 시각이 아니라 횟수로 계약 | S2 계약 | **강수빈**(교대근무 실패자)의 *구조적 배제*<br>**임태경**(상습 미달성자)의 *불규칙 노동* | 쓰고 싶어도 못 쓰는 집단을 복구. 교대·유동 근무자 규모 확인 선행 `[미측정]` |
| **4** | **복구권 기본 지급 + 미달성 사유 태깅** | S4 인증 | **김도현**(크루 러너)의 *무응답형 이탈*<br>**강수빈**(교대근무 실패자)의 *실패 사유 미구분* | 리텐션에 직결되면서 구현 난도가 가장 낮음 |
| **5** | **캡틴 부담 이관** — 리마인드 시스템 발송, 캡틴 화면은 그룹 총계만, 역할 로테이션 | S3 예치<br>S4 인증<br>S6 재계약 | **정하윤**(크루 캡틴)의 *독촉 역할 재발*과 *역할 고정* | 캡틴 번아웃은 개인 이탈이 아니라 **그룹 전체 소멸**로 이어짐 |
| **6** | **누적 손실 상한 + 연속 실패 시 강제 휴식** | S5 정산 | **임태경**(상습 미달성자)의 *손실 순환* | 🔴 사행성 심사 대응과 직결 — 규제 리스크라 우선순위 상향 여지 있음 |
| **7** | **부분 달성자 관점 회고본** — 그룹 대비가 아닌 개인 진척 중심 편집 | S6 회고 | **김도현**(크루 러너)의 *재계약 동력 상실* | 회고본이 성과물이 아니라 실패 증거가 되지 않도록 |
| **8** | **무료 트랙 분리** — 친목 그룹은 S2에서 끊고 과금하지 않음 | S3 예치 | **윤채원**(데일리 크루)의 *관계 화폐화 거부* | 획득 채널을 과금 대상으로 착각하지 않기 |
