...
> ### ⚠️ 아래 프롬프트는 **예제**다 — 그대로 복사해 쓰지 말 것
>
> | | |
> |---|---|
> | **이 표의 성격** | 워크플로우의 각 단계가 **무엇을 요구하는지 보여주는 예시문**이다. 실제 프로젝트에서 쓴 명령이 아니며, 산출물의 근거도 아니다 |
> | **그대로 쓰면 안 되는 이유** | 프롬프트에 **분석 대상·세그먼트·제약**이 들어가지 않으면 일반론적 페르소나가 나온다. 예제의 *"핵심 5명, 확장 3명…"* 같은 수량도 **방법론이 정한 값이 아니라 예시값**이다 |
> | **쓰는 방법** | 각 단계의 **목적**(2열)과 **산출물**(4열)을 요구사항으로 삼아, 프로젝트의 시장·세그먼트·제약을 넣어 **직접 작성한다** |
> | **분석 산출물에는 넣지 않는다** | 프롬프트는 **도구**이지 지식이 아니다. 이 저장소의 분석 결과 문서에는 프롬프트를 싣지 않는 것을 원칙으로 한다 |

| 단계 | 목적 | AI 프롬프트 **예제** `[그대로 사용 금지]` | 산출물 |
| --- | --- | --- | --- |
